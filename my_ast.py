...
class ArrNode(ExprNode):
    """Класс для представления в AST-дереве массивов
        """

    def __init__(self, name: IdentNode, size: IdentNode,
                 row: Optional[int] = None, col: Optional[int] = None, **props) -> None:
        super().__init__(row=row, col=col, **props)
        self.name = name
        self.type = None
        ...
        with suppress(SemanticException):
            self.type = TypeDesc.from_str(str(name))

    def __str__(self) -> str:
        return str(self.name) + "[{}]".format(self.size.to_str())
    ...
    def semantic_check(self, scope: IdentScope) -> None:
        ...
        self.size.semantic_check(scope)


class TypeNode(ExprNode):
    """Класс для представления в AST-дереве типов данных
...
class ArrRefNode(AstNode):
    def __init__(self, arr_name: IdentNode,  *keys: Tuple[ExprNode],
                 row: Optional[int] = None, line: Optional[int] = None, **props):
        super().__init__(row=row, line=line, **props)
        self.arr_name = arr_name
        self.keys = keys
        self.name = "arr_reference"


    @property
    def childs(self) -> Tuple[IdentNode, ...]:
        return (self.arr_name,) + self.keys
...
class FunDeclNode(StmtNode):
    """Класс для представления в AST-дереве объявления функции
       """

    def __init__(self, type_: TypeNode, name: IdentNode, params: Tuple[ParamNode], func_body: StmtListNode,
                 row: Optional[int] = None, col: Optional[int] = None, **props) -> None:
        super().__init__(row=row, col=col, **props)
        self.type = type_
        self.name = name
    ...
    @property
    def childs(self) -> Tuple[AstNode, ...]:
        return _GroupNode(str(self.type), self.name), _GroupNode('params', *self.params), self.func_body
    ...
    def semantic_check(self, scope: IdentScope) -> None:
        ...
        type_ = TypeDesc(base_type_=None, return_type=self.type.type, params=tuple(params))
        ...
        if self.func_body:
            self.func_body.semantic_check(scope)
        self.node_type = TypeDesc.VOID


class VarsDeclNode(StmtNode):
    def __init__(self, type_: TypeNode, *vars_list: Union[IdentNode, 'AssignNode'],
                 row: Optional[int] = None, col: Optional[int] = None, **props) -> None:
        super().__init__(row=row, col=col, **props)
        self.type = type_
...
class TypeConvertNode(ExprNode):
    ...
    @property
    def childs(self) -> Tuple[AstNode, ...]:
        (_GroupNode(str(self.type), self.expr), )
